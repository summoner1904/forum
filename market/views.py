...
def market(request):
    ...
    if request.user.access == False:
        messages.error(
            request,
            "У Вас нет доступа к маркету. Его можно приобрести в настройках профиля.",
        )
        return render(
            request, "market/market.html", {"form": form, "sell": sell, "items": items}
        )
    else:
    ...
    if request.method == "POST":
        if sell.is_valid():
            ...
            try:
                ...
                return render(
                    request,
                    "market/market.html",
                    {"form": form, "sell": sell, "items": items},
                )
        ...
        if form.is_valid():
            ...
            return render(
                request,
                "market/market.html",
                {"products": products, "form": form, "sell": sell},
            )

    return render(
        request, "market/market.html", {"form": form, "sell": sell,
                                        "items": items}
    )
